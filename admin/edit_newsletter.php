...
// Debug information
error_log("Attempting to edit newsletter ID: " . $id);
...
try {
    ...
    error_log("Found newsletter: " . print_r($newsletter, true));
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    error_log("Updating newsletter ID: " . $id);
    error_log("New title: " . $title);
    
    try {
        $stmt = $pdo->prepare("UPDATE newsletters SET title = ?, body = ? WHERE id = ?");
        ...
        if ($result) {
            ...
            throw new Exception("Failed to update newsletter in database");
        }
    } catch (PDOException $e) {
        error_log("Database Error: " . $e->getMessage());
        $error = "Error updating newsletter: " . $e->getMessage();
    } catch (Exception $e) {
        error_log("General Error: " . $e->getMessage());
        $error = $e->getMessage();
    }
...
<head>
    ...
    <title>Edit Newsletter | Admin Dashboard</title>
...
<body class="bg-gray-100">
    ...
    <div class="container mx-auto px-4 py-8">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-2xl font-bold text-gray-800">Edit Newsletter</h1>
            ...
            <form action="" method="POST" class="space-y-6" onsubmit="return validateForm()">
                <div>
                    ...
                    <input type="text" 
                           ...
                           name="title" 
                           required 
                           value="<?php echo htmlspecialchars($newsletter['title']); ?>"
                ...
                <div>
                    ...
                    <textarea id="body" 
                              ...
                              required 
                              class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500"
                              placeholder="Enter newsletter content"><?php echo htmlspecialchars($newsletter['body']); ?></textarea>
                </div>
