...
try {
    ...
    $base_query = "FROM subscribers";
    $params = [];
    
    // Add search condition if search term exists
    if (!empty($search)) {
        $base_query .= " WHERE email LIKE ? OR status LIKE ?";
        $search_term = "%{$search}%";
...
<body class="bg-gray-100">
    ...
    <div class="container mx-auto px-4 py-8">
        ...
        <div class="mb-6">
            <form action="" method="GET" class="flex gap-4">
                ...
                <?php if (!empty($search)): ?>
                    <a href="subscribers.php" class="bg-gray-600 text-white px-6 py-2 rounded-lg hover:bg-gray-700 transition-colors">
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-teal-50">
                            ...
                            <th class="px-6 py-3 text-left text-xs font-medium text-teal-700 uppercase tracking-wider">Subscribed Date</th>
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full 
                                    <?php echo $subscriber['status'] === 'active' ? 'bg-teal-100 text-teal-800' : 'bg-orange-100 text-orange-800'; ?>">
                                    <?php echo ucfirst(htmlspecialchars($subscriber['status'])); ?>
                                            </span>
                                        </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <form action="handlers/subscriber_handler.php" method="POST" class="inline">
                                    ...
                                    <?php else: ?>
                                        <button type="submit" name="action" value="subscribe" 
                                            ...
                                            <i class="fas fa-check mr-1"></i>Subscribe
                                        </button>
                                    <?php endif; ?>
    ...
    <script>
        ...
        function exportToPDF() {
            ...
            const data = <?php echo json_encode(array_map(function($sub) {
                return [
                    $sub['email'],
                    ucfirst($sub['status']),
                    date('M j, Y', strtotime($sub['subscribed_at']))
                ];
            }, $subscribers)); ?>;
        ...
        function exportToExcel() {
            const data = <?php echo json_encode(array_map(function($sub) {
                return [
                    ...
                    'Subscribed Date' => date('M j, Y', strtotime($sub['subscribed_at']))
                ];
            }, $subscribers)); ?>;
